...
| **Updated by** | Phase 2 completion — platform layer (Claude) |
| **Current phase** | **Phase 2 COMPLETE.** Next: Phase 3 — Providers + live catalogue search |
| **Repo** | https://github.com/Ganu0310/Varuna-AI — `main`. Git 2.55; Credential Manager auth working. |
| **Overall status** | 🟢 Phases 0–2 done and verified against real infrastructure: **136 tests pass** (114 JS unit + 35 DB-integration + 11 Python — note unit/integration counted separately). typecheck / lint / format / real-data policy / token sync clean. Full stack smoke-tested live: register → /me → create investigation (real geodesic area 1,918.76 km²) → AOI-limit rejection naming the exact overage. |
...
| **Biggest current risk** | MKLab dataset request not submitted (B-003); demo incident not locked (B-004); **the winding claim in docs 06/12 is factually wrong — 12 is presentation material (D-011, Q-008)** |
| **Next milestone** | Phase 3 — `ProviderClient` (circuit breaker, retry, quota), CDSE OAuth2 + Planetary Computer + ASF clients, provider chains, live `/catalogue/search`. **Needs Tier-A accounts (§15.4).** |
...
| 3 | Providers + live catalogue search | 🔴 | **Next.** Blocked on Tier-A credentials (§15.4) for CDSE/Earthdata; Planetary Computer works anonymously, so the chain can be started against MPC alone. |
